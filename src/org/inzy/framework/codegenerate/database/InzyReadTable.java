package org.inzy.framework.codegenerate.database;
...
import org.inzy.framework.codegenerate.pojo.TableConvert;
import org.inzy.framework.codegenerate.util.CodeResourceUtil;
import org.inzy.framework.codegenerate.util.CodeStringUtils;

public class InzyReadTable {
	private static final Log log = LogFactory.getLog(InzyReadTable.class);
	...
	public static void main(String[] args) throws SQLException {
		try {
			List<Columnt> cls = new InzyReadTable().readTableColumn("person");
			for (Columnt c : cls) {
		...
		System.out.println(ArrayUtils.toString(new InzyReadTable()
				.readAllTableNames()));
	}
		...
		try {
			...
			if (CodeResourceUtil.DATABASE_TYPE.equals("mysql")) {
				this.sql = MessageFormat
						.format("select table_name from information_schema.tables where table_schema={0} and table_type='base table';",
								new Object[] { TableConvert
										.getV(CodeResourceUtil.DATABASE_NAME) });
			}

			if (CodeResourceUtil.DATABASE_TYPE.equals("oracle")) {
				this.sql = "select table_name from user_tables";
			}

			if (CodeResourceUtil.DATABASE_TYPE.equals("postgresql")) {
				this.sql = "select tablename as table_name from pg_tables where schemaname='public'";
			}

			if (CodeResourceUtil.DATABASE_TYPE.equals("sqlserver")) {
				this.sql = "select Name as table_name from SysObjects where XType='U'";
			}

			this.rs = this.stmt.executeQuery(this.sql);
			this.rs.last();
			if (this.rs.getRow() > 0) {
				tableList.add(this.rs.getString(0));
				while (this.rs.previous()) {
					tableList.add(this.rs.getString("table_name"));
				}
			} else {
				throw new SQLException("该数据库中中没有表");
			}
			return tableList;

		} catch (ClassNotFoundException e) {
			throw new SQLException("找不到jdbc驱动：" + CodeResourceUtil.DIVER_NAME,
					e);
	...
	public List<Columnt> readTableColumn(String tableName) throws Exception {
		...
		try {
			...
			if (n > 0) {
				...
				columnt.setNullable(TableConvert.getNullAble(this.rs
						.getString(7)));

				formatFieldClassType(columnt);
				columnt.setFiledComment(StringUtils.isBlank(this.rs
						.getString(3)) ? columnt.getFieldName() : this.rs
						.getString(3));

				String[] ui_filter_fields = new String[0];
				if (CodeResourceUtil.INZY_GENERATE_UI_FILTER_FIELDS != null) {
					ui_filter_fields = CodeResourceUtil.INZY_GENERATE_UI_FILTER_FIELDS
							.toLowerCase().split(",");
				}

				if ((!CodeResourceUtil.INZY_GENERATE_TABLE_ID.equals(columnt
						.getFieldName()))
						&& (!CodeStringUtils.isIn(columnt.getFieldDbName()
								.toLowerCase(), ui_filter_fields))) {
				...
				while (this.rs.previous()) {
					...
					if ((!CodeResourceUtil.INZY_GENERATE_TABLE_ID.equals(po
							.getFieldName()))
							&& (!CodeStringUtils.isIn(po.getFieldDbName()
									.toLowerCase(), ui_filter_fields))) {
						...
						po.setNullable(TableConvert.getNullAble(this.rs
								.getString(7)));

						formatFieldClassType(po);
						po.setFiledComment(StringUtils.isBlank(this.rs
								.getString(3)) ? po.getFieldName() : this.rs
								.getString(3));
						columntList.add(po);
					}
				}
			...
			throw new Exception("找不到jdbc驱动：" + CodeResourceUtil.DIVER_NAME, e);
		} catch (SQLException e) {
			throw e;
		} finally {
			try {
	...
	public List<Columnt> readOriginalTableColumn(String tableName)
		...
		try {
			...
			if (n > 0) {
				...
				columnt.setPrecision(TableConvert.getNullString(this.rs
						.getString(4)));
				columnt.setScale(TableConvert.getNullString(this.rs
						.getString(5)));
				columnt.setCharmaxLength(TableConvert.getNullString(this.rs
						.getString(6)));
				columnt.setNullable(TableConvert.getNullAble(this.rs
						.getString(7)));
				...
				while (this.rs.previous()) {
					...
					po.setPrecision(TableConvert.getNullString(this.rs
							.getString(4)));
					po.setScale(TableConvert.getNullString(this.rs.getString(5)));
					po.setCharmaxLength(TableConvert.getNullString(this.rs
							.getString(6)));
					po.setNullable(TableConvert.getNullAble(this.rs
							.getString(7)));

					po.setFieldType(formatDataType(this.rs.getString(2)
							.toLowerCase(), po.getPrecision(), po.getScale()));

					formatFieldClassType(po);
					po.setFiledComment(StringUtils.isBlank(this.rs.getString(3)) ? po
