...
import org.inzy.framework.web.cgform.engine.FreemarkerHelper;
import org.inzy.framework.web.cgform.entity.config.CgFormFieldEntity;
import org.inzy.framework.web.cgform.entity.config.CgFormHeadEntity;
...
import org.inzy.framework.web.demo.entity.test.TSStudent;
import org.inzy.framework.web.demo.entity.test.TeacherEntity;
import org.inzy.framework.web.system.dao.repair.RepairDao;
import org.inzy.framework.web.system.pojo.base.*;
import org.inzy.framework.web.system.service.RepairService;
...
public class RepairServiceImpl extends CommonServiceImpl implements
	...
	public void deleteAndRepair() {
		...
		commonDao.executeHql("delete InzyDemoCkfinderEntity");
		...
		commonDao.executeHql("delete InzyJdbcEntity ");
		commonDao.executeHql("delete InzyOrderMainEntity ");
		commonDao.executeHql("delete InzyOrderProductEntity ");
		commonDao.executeHql("delete InzyOrderCustomEntity ");
		commonDao.executeHql("delete InzyNoteEntity ");
		commonDao.executeHql("update InzyMatterBom mb set mb.parent = null");
		commonDao.executeHql("delete InzyMatterBom ");
		repair();
	}
	...
	private void repairMatterBom() {
		InzyMatterBom entity = new InzyMatterBom();
		entity.setCode("001");
		entity.setName("电脑");
		entity.setUnit("台");
		...
		InzyMatterBom entity2 = new InzyMatterBom();
	...
	private void repairOrder() {
		InzyOrderMainEntity main = new InzyOrderMainEntity();
		...
		main.setUsertype("1");
		commonDao.save(main);
		InzyOrderProductEntity product = new InzyOrderProductEntity();
		...
		InzyOrderCustomEntity coustom = new InzyOrderCustomEntity();
	 ...
	 *@date   2013-11-12
	 */
	private void repairInzyNoteEntity() {
		InzyNoteEntity entity = new InzyNoteEntity();
		entity.setAge(10);
...
//		commonDao.saveOrUpdate(jform_cgreport_item_field_type);
	...
	private void repairLog() {
		...
		try {
			...
			log6.setLogcontent("InzyDemo例子: 12被删除 成功");
			...
			log7.setLogcontent("InzyDemo例子: 12被删除 成功");
			...
			log8.setLogcontent("InzyDemo例子: 12被删除 成功");
			...
			log16.setLogcontent("InzyDemo例子: 12被添加成功");
	...
	private void repairDemo() {
		...
		// 复杂字符串文本读取，采用文件方式存储
		String html = new FreemarkerHelper().parseTemplate("/org/inzy/framework/web/system/txt/valid-code-demo.ftl", null);
...
//		admin.setTSDepart(eiu);
		...
		scott.setRealName("Goodman Zhang");
...
//		approver.setTSDepart(eiu);
	...
	private void repairAttachment() {
		...
		treaty.setAttachmenttitle("INZY平台协议");
		treaty.setRealpath("INZY平台协议");
		...
		analyse.setAttachmenttitle("分析INZY与其他的开源项目的不足和优势");
		analyse.setRealpath("分析INZY与其他的开源项目的不足和优势");
		...
		standard.setAttachmenttitle("INZY团队开发规范");
		standard.setRealpath("INZY团队开发规范");
		...
		settable.setAttachmenttitle("（Goodman Zhang）-金融服务公司机构岗位职责与任职资格设置表(根据模板填写）");
		settable.setRealpath("（Goodman Zhang）-金融服务公司机构岗位职责与任职资格设置表(根据模板填写）");
		...
		taghelp.setAttachmenttitle("INZYUI标签库帮助文档v3.2");
		taghelp.setRealpath("INZYUI标签库帮助文档v3.2");
	...
	private void repaireIcon() {
		org.inzy.framework.core.util.LogUtil.info("修复图标中");
	...
	private void repairMenu() {
		...
		TSIcon folder = commonDao.findByProperty(TSIcon.class, "iconName",
				"文件夹").get(0);
		org.inzy.framework.core.util.LogUtil.info(defaultIcon.getIconPath());
		...
		demo.setFunctionUrl("inzyDemoController.do?inzyDemo");
		...
		minidao.setFunctionUrl("inzyMinidaoController.do?inzyMinidao");
		...
		onetable.setFunctionUrl("inzyNoteController.do?inzyNote");
		...
		onetoMany.setFunctionUrl("inzyOrderMainController.do?inzyOrderMain");
		...
		jdbcdemo.setFunctionUrl("inzyJdbcController.do?inzyJdbc");
		...
		sqlsep.setFunctionUrl("inzyJdbcController.do?dictParameter");
		...
		notag1.setFunctionUrl("inzyEasyUIController.do?inzyEasyUI");
		...
		notag2.setFunctionUrl("inzyOrderMainNoTagController.do?inzyOrderMainNoTag");
		...
		htmledit.setFunctionUrl("inzyDemoController.do?ckeditor&isIframe");
		...
		matterBom.setFunctionUrl("inzyMatterBomController.do?goList");
		...
		ckfinder.setFunctionUrl("inzyDemoCkfinderController.do?inzyDemoCkfinder");
