package org.inzy.framework.web.cgform.controller.generate;
...
import org.inzy.framework.codegenerate.generate.CgformCodeGenerate;
import org.inzy.framework.codegenerate.generate.onetomany.CgformCodeGenerateOneToMany;
...
import org.inzy.framework.web.cgform.entity.generate.GenerateSubListEntity;
import org.inzy.framework.web.cgform.service.button.CgformButtonServiceI;
import org.inzy.framework.web.cgform.service.button.CgformButtonSqlServiceI;
import org.inzy.framework.web.cgform.service.cgformftl.CgformFtlServiceI;
import org.inzy.framework.web.cgform.service.config.CgFormFieldServiceI;
import org.inzy.framework.web.cgform.service.enhance.CgformEnhanceJsServiceI;
...
public class GenerateController extends BaseController {
	...
	public ModelAndView gogenerate( CgFormHeadEntity cgFormHead,HttpServletRequest request) {
		...
		if(cgFormHead.getJformType()==1 || cgFormHead.getJformType()==3){
			//如果是单表或者附表，则进入单表模型
			returnModelAndView = "inzy/cgform/generate/single";
		}else{
			//如果是主表，则进入一对多模型
			...
			if(StringUtil.isNotEmpty(cgFormHead.getSubTableStr())){
				...
				for(String subTable :subTables){
					...
					entityNameMap.put(subHead.getTableName(), InzyReadTable.formatFieldCapital(subHead.getTableName()));
				}
			}
			request.setAttribute("subTableList", subTableList);
			returnModelAndView = "inzy/cgform/generate/one2many";
		}
		...
		String entityName = InzyReadTable.formatFieldCapital(cgFormHead.getTableName());
	...
	public void dogenerate(CgFormHeadEntity cgFormHead,GenerateEntity generateEntity,CreateFileProperty createFileProperty,
		...
		try {
			//step.2 判断表是否存在
			boolean tableexist = new InzyReadTable().checkTableExist(tableName);
			if(tableexist){
	...
	private String replaceAddJSP(String cgformJspHtml) {
		...
		while (cgformJspHtml.indexOf("#{") > 0) {
			...
			cgformJspHtml = cgformJspHtml.replace(key, "<input id='"+InzyReadTable.formatField(realKey)+"' name='"+InzyReadTable.formatField(realKey)+"' type='text' value='${'$'}{${entityName?uncap_first}Page."+InzyReadTable.formatField(realKey)+"}' style='width: 150px' class='inputxt' >");
		}
	...
	private void getCgformConfig(CgFormHeadEntity cgFormHead,
		...
		for(CgFormFieldEntity field : cgFormHead.getColumns()){
			...
			if(listJsCopy!=null){
				listJsCopy.setCgJsStr(listJsCopy.getCgJsStr().replace(fieldName, InzyReadTable.formatField(fieldName)));
			}
			if(formJsCopy!=null){
				formJsCopy.setCgJsStr(formJsCopy.getCgJsStr().replace(fieldName, InzyReadTable.formatField(fieldName)));
			}
		}
	...
	public ModelAndView goFileTree(HttpServletRequest request) {
		return new ModelAndView("inzy/cgform/generate/fileTree");
	}
	/**
	 * 返回子目录json
	 * @param parentNode
