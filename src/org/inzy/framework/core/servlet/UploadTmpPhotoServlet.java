package org.inzy.framework.core.servlet;
		...
		try {
			...
			for (int i = 0; i < items.size(); i++) {
				...
				if (!item.isFormField() && item.getName().length() > 0) {
					...
					// org.inzy.framework.core.util.LogUtil.info(item.getName()+"=="+UploadTool.createPhotoID()+suffixName+"=="+UploadTool.FormetFileSize(item.getSize())+"savePath"+savePath);
