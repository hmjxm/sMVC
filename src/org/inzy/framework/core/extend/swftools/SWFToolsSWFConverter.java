...
import org.inzy.framework.core.util.PinyinUtil;
...
public class SWFToolsSWFConverter implements SWFConverter {
	...
	public void convert2SWF(String inputFile, String swfFile, String extend) {
		...
		if (!pdfFile.exists()) {
			 org.inzy.framework.core.util.LogUtil.info("PDF文件不存在！");
			return;
		}
	
		if (outFile.exists()) {
			 org.inzy.framework.core.util.LogUtil.info("SWF文件已存在！");
			return;
		}
...
//				+ "\" -o " + swfFile + " -s languagedir=D:\\xpdf-chinese-simplified -T 9 -f";
		...
		try {
			...
			try {
				process.waitFor();
				org.inzy.framework.core.util.LogUtil.info("时间-------"+process.waitFor());
