package org.inzy.framework.core.common.dao.impl;
...
import org.inzy.framework.core.annotation.InzyEntityTitle;
import org.inzy.framework.core.common.dao.IGenericBaseCommonDao;
import org.inzy.framework.core.common.dao.jdbc.JdbcDao;
...
import org.inzy.framework.core.common.hibernate.qbc.DetachedCriteriaUtil;
import org.inzy.framework.core.common.hibernate.qbc.HqlQuery;
import org.inzy.framework.core.common.hibernate.qbc.PageList;
import org.inzy.framework.core.common.hibernate.qbc.PagerUtil;
...
import org.inzy.framework.core.util.ToEntityUtil;
...
public abstract class GenericBaseCommonDao<T, PK extends Serializable>
	...
	private <T> void getProperty(Class entityName) {
		...
		for (int i = 0; i < str.length; i++) {
			...
			org.inzy.framework.core.util.LogUtil.info(property + "---&gt;" + type);
		}
	}
	...
	public List<DBTable> getAllDbTableName() {
		...
		for (String key : (Set<String>) metaMap.keySet()) {
			...
			try {
				c = Class.forName(key);
				InzyEntityTitle t = c.getAnnotation(InzyEntityTitle.class);
	 ...
	 * @return
	 */
	public List<T> findByQueryString(final String hql, Object... objs) {
	...
	public <T> List<T> findByQueryStringParam(String hql, Map<String, Object> param) {
		...
		if (param != null && param.size() > 0) {
			...
			while (iter.hasNext()) {
				...
				sKey = StringUtils.defaultString((String) entry.getKey(), "");
				queryObject.setParameter(sKey, entry.getValue());
			}
		}
	 ...
	 * @return
	 */
	public List<T> findListbySql(final String query, Object... objs) {
	...
	public <T> List<T> findListbySqlParam(String query,
		...
		if (param != null && param.size() > 0) {
			...
			while (iter.hasNext()) {
				Map.Entry entry = (Map.Entry) iter.next();
				sKey = StringUtils.defaultString((String) entry.getKey(), "");
				querys.setParameter(sKey, entry.getValue());
			}
		}
	...
	public List<Map<String, Object>> findForJdbc(String sql, int page, int rows) {
		// 封装分页SQL
		sql = JdbcDao.inzyCreatePageSql(sql, page, rows);
	...
	public <T> List<T> findObjForJdbc(String sql, int page, int rows,
		...
		// ===================================================//
		// Add by yun.tang 2015/06/04：判断objs是否为null
		if (objs == null || objs.length <= 0) {
			mapList = jdbcTemplate.queryForList(sql);
		} else {
			mapList = jdbcTemplate.queryForList(sql, objs);
		}
		// ===================================================//
	...
	public <T> List<T> findObjForJdbcParam(String sql, int page, int rows,
		...
		// ===================================================//
		// Add by yun.tang 2015/06/04：判断param是否为null
		if (param == null || param.size() <= 0) {
			mapList = jdbcTemplate.queryForList(sql);
		} else {
			mapList = namedParameterJdbcTemplate.queryForList(sql, param);
		}
		// ===================================================//
	...
	public List<Map<String, Object>> findForJdbcParam(String sql, int page,
		...
		// ===================================================//
		// Modify by yun.tang 2015/06/26：判断param是否为null
	...
	public Long getCountForJdbcParam(String sql, Map<String, Object> param) {
		...
		if (param == null || param.size() <= 0) {
			...
			return this.namedParameterJdbcTemplate.queryForLong(sql, param);
		}
	}

	/**
	 * 使用指定的检索标准检索并返回数据（不分页）
	 * 
	 * @param sql
	 * @param objs
	 * @return 
	 ...
	 * @return
	 */
	public <T> List<T> findObjForJdbc(String sql, RowMapper<T> rowMapper,
			Object... objs) {
		List<T> rsList = new ArrayList<T>();

		rsList = jdbcTemplate.query(sql, rowMapper, objs);
		return rsList;
	}
	...
	public <T> List<T> findObjForJdbc(String sql, Class<T> clazz,
		...
		rsList = jdbcTemplate
				.query(sql, new BeanPropertyRowMapper(clazz), objs);
		return rsList;
	...
	public <T> List<T> findObjForJdbcParam(String sql, Class<T> clazz,
		...
		// List<T> rsList = new ArrayList<T>();
		// rsList = jdbcTemplate.query(sql, new BeanPropertyRowMapper(clazz),
		// objs);
		// return rsList;
