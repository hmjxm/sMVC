...
<html>
	...
	<table cellpadding="0" cellspacing="1" class="formtable">
		...
		<#if !po_has_next>
			...
			<div style="width: auto;height: 200px;">
				...
				<#list subTab as sub>
				 <t:tab href="${entityName?uncap_first}Controller.do?${sub.entityName?uncap_first}List<#list sub.foreignKeys as key><#if key?lower_case?index_of("${inzy_table_id}")!=-1>&${inzy_table_id}=${"$"}{${entityName?uncap_first}Page.${inzy_table_id}}<#else>&${key?uncap_first}=${"$"}{${entityName?uncap_first}Page.${key?uncap_first}}</#if></#list>" icon="icon-search" title="${sub.ftlDescription}" id="${sub.entityName?uncap_first}"></t:tab>
				</#list>
				</t:tabs>
			</div>
			</t:formvalid>
 ...
 <script src = "webpage/${bussiPackage?replace('.','/')}/${entityPackage?replace(".","/")}/${entityName?uncap_first}.js"></script>	